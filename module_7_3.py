"""
Задача "Найдёт везде": Напишите класс WordsFinder, объекты которого создаются следующим образом: WordsFinder(
'file1.txt, file2.txt', 'file3.txt', ...). Объект этого класса должен принимать при создании неограниченного
...
class WordsFinder:
    ...
    def del_punctuation_in_line(self, line):
        mask = [',', '.', '=', '!', '?', ';', ':', ' - ']  # символы для удаления в строке
        new_line = line
        for symb in mask:
            new_line = new_line.replace(symb, '')
        return new_line
        ...
        for name in self.files:
            if os.path.exists(name):
                with open(name, 'r', encoding='utf-8') as file:
                    ...
                    for line in file:
                        line = line.lower()
                        words += (self.del_punctuation_in_line(line)).split()
    ...
    def find(self, word):  # Возвращает словарь, где ключ - название файла, значение - позиция первого такого слова
        # в списке слов этого файла.
        dictionary_words = self.get_all_words()
        find_dict = []
        word = word.lower()
        for element in dictionary_words:
            for key, words in element.items():
                if word in words:
                    find_dict.append({key: words.index(word) + 1})
        return find_dict

    def count(self, word):  # метод, где word - искомое слово. Возвращает словарь, где ключ - название файла, значение -
        # количество слова word в списке слов этого файла.
        dictionary_words = self.get_all_words()
        find_count = []
        word = word.lower()
        for element in dictionary_words:
            for key, words in element.items():
                if word in words:
                    find_count.append({key: words.count(word)})
        return find_count
